...
AI-powered code review in your terminal. Run `specter review` inside any git repository and Specter collects your branch's diff, sends it to a locally installed AI agent (`claude`, `codex`, `opencode`, or `pi`), validates the findings, checks your coding standards, and renders a color-coded report.
...
- **One AI coding CLI** on your `PATH`:
  ...
  - [`pi`](https://pi.dev) — Pi coding agent (supports 30+ providers)
- **`git`** on your `PATH`
...
specter review --orchestrated           # multi-pass sub-agent review (auto-enabled with phenomenal)
specter review --max-depth 3            # orchestrator recursion depth (default 2)
specter review --max-parallel 5         # concurrent sub-agents (default 3)
specter review --no-validate            # skip the validation pass (faster)
specter review --no-standards           # skip the standards check
specter review --no-auto-learn          # skip auto-updating CLAUDE.md after review
specter review --agent pi               # pick a specific AI agent
...
### Orchestrated multi-pass review

For large branches with many files or complex changes, enable orchestrated mode:

```powershell
specter review --orchestrated                  # multi-pass: survey → deep-dives → merge
specter review --depth phenomenal               # phenomenal auto-enables orchestrated mode
specter review --orchestrated --max-depth 3     # deeper recursion for very large changes
specter review --orchestrated --max-parallel 5  # more concurrent sub-agents
```

The orchestrator runs in multiple phases:

1. **Survey** — The agent analyzes the full diff and identifies hotspot areas that need deep investigation (not bugs yet — just areas of concern with file lists, focus descriptions, and priorities).
2. **Deep-dives** — Each hotspot area gets its own focused agent call with full file contents, relevant diff hunks, and repo context. These run in parallel (up to `--max-parallel`).
3. **Sub-dives** — Each deep-dive can discover sub-areas that need further investigation. The orchestrator recursively explores these up to `--max-depth` levels.
4. **Merge** — All findings are deduplicated by `(file, line, normalized title)` and ranked by severity.

Budget guard: total agent calls are capped at 15 to prevent runaway costs.
...
After each review, Specter automatically fires a Claude Code task that reads the confirmed findings and updates your repo's `CLAUDE.md` with concise guidance to prevent the same mistakes in future code. If a finding warrants a reusable checklist or template, it may also create a skill file under `.claude/commands/`.

This runs silently after the interactive copy prompt and requires `claude` (Claude Code CLI) to be installed and authenticated. If `claude` is not on your PATH the step is skipped with no error.

**Disable for a single review:**

```powershell
specter review --no-auto-learn
```

**Disable permanently via Options:**

```
specter          → Options → Auto-learn from findings: OFF
```
...
|---|---|---|
| Agent | `claude` | `--agent claude/codex/opencode/pi` |
...
| Auto-learn | enabled | `--no-auto-learn` |
...
```powershell
specter review --agent pi                           # uses default model
specter review --agent pi --model anthropic/claude-sonnet-4
specter review --agent pi --model google/gemini-2.5-pro
specter review --agent pi --model openai/gpt-4.1
```

Requires [`pi`](https://pi.dev) CLI installed (`npm install -g @earendil-works/pi-coding-agent`). Pi supports 30+ providers (Anthropic, OpenAI, Google, DeepSeek, and many more) and is configured via API keys or OAuth login. Run `specter` → Options → Change model to browse all available models grouped by provider.
